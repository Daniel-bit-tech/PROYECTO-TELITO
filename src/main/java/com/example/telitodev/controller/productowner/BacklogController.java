...
public class BacklogController {
    ...
    public String showBacklogView(Model model, Authentication auth, HttpSession session,
                                  ...
                                  @RequestParam(name = "estado", required = false, defaultValue = "") String estado) { // Nuevo parámetro
        ...
        if (q != null && !q.trim().isEmpty()) {
            // Hay búsqueda por texto
            if ("Resuelto".equals(estado)) {
                backlogPage = backlogRepository.findResueltosBySearchTerm(q.trim(), pageable);
            } else if ("No Resuelto".equals(estado)) {
                backlogPage = backlogRepository.findNoResueltosBySearchTerm(q.trim(), pageable);
            } else {
            ...
            if ("Resuelto".equals(estado)) {
                backlogPage = backlogRepository.findByEstadoBacklog("Resuelto", pageable);
            } else if ("No Resuelto".equals(estado)) {
                backlogPage = backlogRepository.findByEstadoBacklogNot("Resuelto", pageable);
            } else {
        ...
        model.addAttribute("estadoFiltro", estado); // Nuevo atributo para el filtro
