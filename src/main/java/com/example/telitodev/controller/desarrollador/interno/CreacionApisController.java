...
public class CreacionApisController extends BaseController {
    ...
    public CreacionApisController(ContratoApiService contratoApiService, DominioRepository dominioRepository, TagRepository tagRepository, ContratoRepository contratoRepository, ApiRepository apiRepository, EstadoApiRepository estadoApiRepository, VersionApiRepository versionApiRepository) {
    ...
    public String vistaCrearVersionApi(Model model, Authentication auth, HttpSession session, @PathVariable("idApi") Integer idApi) {
        ...
        if (apiX.isPresent() && apiX.get().getUsuario().equals(usuario)) {
            ...
            if (api.getVersionesApi().isEmpty()) {
                ...
                versionContratoDto.setVersion("v1.0");
            }
